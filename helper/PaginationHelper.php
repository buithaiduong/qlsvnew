...
if(!function_exists('createLink')){
    function createLink($data = []){
        // tao duong link phan trang cho tung chuc nang cua he thong
        /*
...
if(!function_exists('pagination')){
    function pagination($link, $totalItems, $page = 1, $limit = 2){
        ...
        // ceil : lam tron so trong php (lam tron so len)
        // gioi han lai $page
        ...
        if($page > 1){
            ...
            $htmlPage .= '<a style="color:;" href="'.str_replace('{page}', $page-1, $link).'" class="page-link">Previous</a>';
            $htmlPage .= '</li>';
        }
        // xu ly cac trang
        for($i = 1; $i <= $totalPage; $i++){
            if($i == $page){
                //thong bao cho nguoi dung biet dang o trang nao
                $htmlPage .= '<li class="page-item active" aria-current="page">';
                $htmlPage .= '<a style="background-color:dodgerblue;" class="page-link">'.$page.'</a>';
                $htmlPage .= '</li>';
            } else {
                // cac trang khac
                $htmlPage .= '<li class="page-item">';
                $htmlPage .= '<a style="color: dodgerblue;" href="'.str_replace('{page}', $i, $link).'" class="page-link">'.$i.'</a>';
                $htmlPage .= '</li>';
            }
        }
        ...
        if($page < $totalPage){
            $htmlPage .= '<li class="page-item">';
            $htmlPage .= '<a style="color: dodgerblue;" class="page-link" href="'.str_replace('{page}', $page+1, $link).'">Next</a>';
            $htmlPage .= '</li>';
        }
        $htmlPage .= '</ul>';
        $htmlPage .= '</nav>';
